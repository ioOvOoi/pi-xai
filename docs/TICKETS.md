...
| 3 | 模型注册 | Grok Build 模型目录 → `XaiLlmAdapter`（Responses SSE→StreamChunk）+ 请求归一化 + CLI 代理头 + settings 段 | 模型出现在 DSH 模型列表；流式对话可用 | ⬜ |
| 4 | 文本工具 | xai_generate_text / xai_multi_agent / xai_x_search | 工具注册成功、schema 校验、真实调用（mock/线上） | ⬜ |
